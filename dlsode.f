...
C**End
     ...
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU,
     4   NNI, NCF, NETF
      INTEGER I, I1, IREDO, IRET, J, JB, M, NCFL, NEWQ
...
C-----------------------------------------------------------------------
 200  IF (ABS(RC-1.0D0) .GT. CCMAX) IPUP = MITER
      ...
      WRITE(*,*) '[DLSODE][update-tn] TN = ', TN
      WRITE(*,*) '[DLSODE][update-tn] H = ', H
      WRITE(*,*) '[DLSODE][update-tn] NQ = ', NQ
      TN = TN + H
      I1 = NQNYH + 1
      DO 215 JB = 1,NQ
        I1 = I1 - NYH
...
C-----------------------------------------------------------------------
 ...
 290    Y(I) = SAVF(I) - ACOR(I)
      DEL = DVNORM (N, Y, EWT)
      WRITE(*,*) '[DLSODE][functional-iteration] DEL = ', DEL
      DO 300 I = 1,N
...
C-----------------------------------------------------------------------
 ...
 360    Y(I) = H*SAVF(I) - (YH(I,2) + ACOR(I))
      ...
      DEL = DVNORM (N, Y, EWT)
      WRITE(*,*) '[DLSODE][chord-method] DEL = ', DEL
      DO 380 I = 1,N
        ACOR(I) = ACOR(I) + Y(I)
...
C-----------------------------------------------------------------------
 400  IF (M .NE. 0) CRATE = MAX(0.2D0*CRATE,DEL/DELP)
      WRITE(*,*) '[DLSODE][conv-test] CRATE = ', CRATE
      NNI = NNI + 1
      WRITE(*,*) '[DLSODE][conv-test] TQ = ', TESCO(2,NQ)
      WRITE(*,*) '[DLSODE][conv-test] CONIT = ', CONIT
      WRITE(*,*) '[DLSODE][conv-test] TOL = ', (TESCO(2,NQ)*CONIT)
      DCON = DEL*MIN(1.0D0,1.5D0*CRATE)/(TESCO(2,NQ)*CONIT)
      ! DCON = DEL*MIN(1.0D0,1.5D0*CRATE)/(TESCO(2,NQ)*0.1D0)
      WRITE(*,*) '[DLSODE][conv-test] DCON = ', DCON
...
C-----------------------------------------------------------------------
      WRITE(*,*) '[DLSODE][conv-test-fail] DELP = ', DELP
...
C-----------------------------------------------------------------------
 450  JCUR = 0
      ...
      WRITE(*,*) '[DLSODE][local-err-test]: DSM = ', DSM
